...
import { RemindthreePage } from '../remindthree/remindthree';
...
@IonicPage()
@Component({
  selector: 'page-scanthree',
  templateUrl: 'scanthree.html',
})
export class ScanthreePage {
  scanFormat: string;
  card_id: any;
  studentArray = [];
  term: any;
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad ScanthreePage');
  }
  ...
  postJsonData() {
    ...
    this.http.post(url, jsonData).subscribe((data: any) => {
      ...
      this.navCtrl.push(ScanthreePage)
      console.log(data);
    }
    );
  }
  ...
  remind() {
    this.navCtrl.push(RemindthreePage)
  }
